...
import { Eye, Trash, X, Calendar, Clock, Users, Monitor, Star } from "lucide-react";
        ...
        <div className='bg-[#121212] flex font-[Poppins] overflow-hidden min-h-screen'>
            ...
            <div className='w-full min-h-screen text-white px-7 py-3 pt-7 ml-[65px]'>
                ...
                <div className='space-y-4 mt-6'>
                    {movies.map(movie => (
                        <div key={movie.id} className='rounded-lg bg-[#1a1a1a] border border-[#2a2a2a] overflow-hidden hover:border-[#3a3a3a] transition-all'>
                            <div className='flex gap-4 p-4'>
                                ...
                                <div className='flex-shrink-0'>
                                    <img 
                                        src={movie.poster} 
                                        alt={movie.title}
                                        className='w-[110px] h-[160px] object-cover rounded-md'
                                    />
                                </div>
                                ...
                                <div className='flex-1 flex flex-col justify-between'>
                                    <div>
                                        <div className='flex items-start justify-between mb-2'>
                                            <div>
                                                <h3 className='text-[16px] text-white font-medium mb-1'>{movie.title}</h3>
                                                <div className='flex items-center gap-2 mb-3'>
                                                    ...
                                                    {movie.rating > 0 && (
                                                        <div className='flex items-center gap-1 ml-1'>
                                                            ...
                                                            <span className='text-[11px] text-[#aaa]'>{movie.rating}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        <div className='grid grid-cols-2 gap-x-6 gap-y-1.5 text-[12px]'>
                                            <div className='flex items-center gap-2'>
                                                ...
                                                <span className='text-[#ccc]'>{movie.duration}</span>
                                            </div>
                                            <div className='flex items-center gap-2'>
                                                <Calendar className='w-3.5 h-3.5 text-gray-500' />
                                                <span className='text-[#888]'>Release:</span>
                                                <span className='text-[#ccc]'>{movie.releaseDate}</span>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className='h-full flex flex-col'>
                                            ...
                                            <div className='mb-4'>
                                                ...
                                                <div className='text-[10px] text-[#666] mt-0.5'>
                                                    {movie.totalBookings > 2000 ? 'Excellent performance' : 
                                                     movie.totalBookings > 1500 ? 'Good performance' : 
                                                     'Average performance'}
                                                </div>
                                            </div>
                                            ...
                                            <div className='flex-1'>
                                                <div className='flex items-center justify-between mb-2'>
                                                    <div className='flex items-center gap-2'>
                                                        ...
                                                        <span className='text-[11px] text-[#888]'>Screen Showings</span>
                                                    </div>
                                                    <span className='text-[12px] font-semibold text-white'>
                                                        {getTotalShowings(movie.screenShowings)} shows
                                                    </span>
                                                </div>
                                                <div className='space-y-1.5 bg-[#0f0f0f] rounded p-2 max-h-[80px] overflow-y-auto'>
                                                    {movie.screenShowings.map((screen, idx) => (
                                                        <div key={idx} className='flex items-center justify-between text-[11px]'>
                                                            <span className='text-[#999]'>{screen.location}</span>
                                                            <span className='text-[#ccc] font-medium'>{screen.showings} shows</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
