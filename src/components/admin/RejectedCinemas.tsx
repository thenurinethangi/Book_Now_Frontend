...
import { MapPin, Tv, Star, CircleFadingArrowUp, CircleX, BrushCleaning, Trash, Trash2, Minus } from "lucide-react";
import { deleteRejectedCinema, getAllActiveCinemas, getAllRejectedCinemas, makeCinemaApprove } from '../../services/admin/cinemaService';
...
function RejectedCinemas(props: any) {
    ...
    useEffect(() => {
        loadAllRejectedCinemas();
    }, []);

    async function loadAllRejectedCinemas() {
        try {
    ...
    function deleteACinema(e: React.MouseEvent<HTMLButtonElement>) {
        const id: any = e.currentTarget.dataset.cinema;

        askConfirm(async () => {
            try { 
                const res = await deleteRejectedCinema(id);
                toast.success(`Permanently deleted rejected Cinema ID ${id}`);
                loadAllRejectedCinemas();
            }
            catch (e) {
                toast.error(`Failed to detele Cinema ID, ${id}, please try again later!`);
            }
        }, 'Delete Rejected Cinema? ', 'This action will permanently remove the rejected cinema request. This cannot be undone.');
    }
                    ...
                    <div className='h-[45%] relative overflow-hidden'>
                        <div className='absolute top-0 left-0 right-0 p-2.5 flex justify-end items-start z-10'>
                            ...
                            <div className='flex items-center gap-1'>
                                ...
                                <button onClick={deleteACinema} data-cinema={cinema._id} className='w-7 h-7 bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-black/70 transition-colors cursor-pointer'>
                                    <Minus className='absolute text-red-600/90 w-[19px] h-[19px]' />
                                </button>
                            </div>
                        </div>
...
export default RejectedCinemas