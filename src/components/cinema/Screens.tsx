import { Users, Maximize, Trash2, Edit3, CheckCircle, XCircle } from 'lucide-react';
import { useEffect, useState } from 'react';
import { getAllScreens } from '../../services/cinema/screenService';
    ...
    return (
        <div className='grid grid-cols-2 gap-5'>
            {screens.map((screen: any, index: number) => (
                <div key={screen._id} className='rounded-lg bg-[#1e1e1e] border border-gray-800 hover:border-gray-700 transition-all duration-300 overflow-hidden group relative'>
                    <div className='h-[200px] relative overflow-hidden'>
                        <div className='absolute top-0 left-0 right-0 p-4 flex justify-between items-start z-10'>
                            ...
                            <button onClick={(e) => {setActiveOptionsId(screen._id); e.stopPropagation()}} className='w-8 h-8 bg-black/40 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-black/60 transition-colors'>
                    ...
                    <div className='p-4.5'>
                        <div className='flex items-start justify-between mb-3'>
                            <div>
                                <h3 className='text-[18px] text-[#dedede] font-medium mb-1'>{screen.screenName}</h3>
                        ...
                        <div className='flex items-center gap-4 mb-4'>
                            <div className='flex items-center gap-1.5'>
                                ...
                                <span className='text-[12px] text-[#999]'>{screen.numberOfSeats} seats</span>
                            </div>
                            {screen.status === 'ACTIVE' && (
                                <div className='flex items-center gap-1.5'>
                                    <div className='w-1.5 h-1.5 bg-green-500 rounded-full' />
                                    <span className='text-[12px] text-green-500'>100 bookings today</span>
                                </div>
                            )}
                        </div>
                    ...
                    <div className={`flex flex-col shadow-2xl absolute top-10 right-0 z-[300] bg-[#1e1e1e] border border-gray-700 rounded-md overflow-hidden min-w-[160px] ${activeOptionsId && activeOptionsId === screen._id ? '' : 'hidden'}`}>
                        ...
                        >
                            ...
                            <p className='text-[13px] text-gray-300 group-hover:text-white'>Edit</p>
                        </button>
                        ...
                        >
                            {screen.status === 'ACTIVE' ? (
                                <>
                                    ...
                                    <p className='text-[13px] text-gray-300 group-hover:text-white'>Mark Unavailable</p>
                                </>
                            ) : (
                                <>
                                    <CheckCircle className='w-4 h-4 text-green-400 group-hover:text-green-300' />
                                    <p className='text-[13px] text-gray-300 group-hover:text-white'>Mark Available</p>
                                </>
                            )}
                        </button>
                        ...
                        {/* Delete Option */}
                        <button onClick={handleDeleteScreen}
                            className='flex items-center gap-3 px-4 py-2.5 hover:bg-red-900/20 transition-colors group'
                        >
                            <Trash2 className='w-4 h-4 text-red-400 group-hover:text-red-300' />
                            <p className='text-[13px] text-gray-300 group-hover:text-red-300'>Delete</p>
                        </button>
                    </div>
